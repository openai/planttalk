...
export const usePlantSensorsStore = create<PlantSensorsState>()((set) => {
  ...
  async function connectToPort(nextPort: SerialPortLike, readyMessage: string) {
    ...
    try {
      await closePort("Connect Arduino or use the fallback sliders.");
  ...
  return {
    ...
    statusMessage: "Connect Arduino or use the fallback sliders.",
    ...
    connect: async () => {
      ...
      if (!serial) {
        updateConnection("unsupported", "Web Serial is unavailable in this browser. Use the fallback sliders.");
        return;
      }
      ...
      try {
        ...
        if (isAbortError(error)) {
          updateConnection("disconnected", "Arduino selection cancelled. Using slider fallback.");
          return;
        }

        updateConnection(
          "disconnected",
          error instanceof Error ? error.message : "Unable to connect to Arduino. Using slider fallback.",
        );
      }
    },
    ...
    disconnect: async () => {
      ...
      stopReconnecting();
      await closePort("Arduino disconnected. Using slider fallback.");
    },
    ...
    init: () => {
      ...
      updateConnection("disconnected", "Connect Arduino or use the fallback sliders.");
...
// Hardware readings only count while the Arduino is actually connected and has
// delivered data. After a disconnect we fall back to the sliders rather than
// silently reporting frozen stale values forever.
function hasUsableHardwareReadings(state: PlantSensorsState) {
  return state.connectionState === "connected" && state.lastHardwareReadingAt !== null;
}
...
function describeOpenError(error: unknown): Error {
  ...
  return new Error(
    rawMessage
      ...
      : "Couldn't open the Arduino. Check the cable and try again, or use the fallback sliders.",
  );
}
