...
If you do not have an Arduino yet, you can still run the app. Choose
**Manual sliders** in the dashboard's **Sensor mode** control, then try the
conversation first and add hardware later.
...
5. Select the port: **Tools -> Port -> COM3**, **/dev/cu.usbmodem...**, or
   whichever port says Arduino next to it.
...
2. If needed, select **Arduino sensors** under **Sensor mode**.
3. When the browser asks you to select a serial port, choose the one labeled
